...
if (isset($_POST['simpan'])) {
    ...
    if ($tipe_file == "application/pdf") {
        ...
        $sql = "INSERT INTO tb_perda  (nama, tahun, subag) VALUES ('$nama', '$tahun','$subag')";
        mysqli_query($koneksi, $sql); //simpan data judul dahulu untuk mendapatkan id

        //dapatkan id terkahir
        $query = mysqli_query($koneksi, "SELECT nama, tgl_upload FROM tb_perda  ORDER BY id_perda DESC LIMIT 1");
        ...
        mysqli_query($koneksi, "UPDATE tb_perda  SET file='$nama_baru' WHERE nama='$data[nama]' ");
//ganti nama file
        $query = mysqli_query($koneksi, "SELECT nama, tgl_upload FROM tb_perda  ORDER BY id_perda DESC LIMIT 1");
        $data = mysqli_fetch_array($query);
