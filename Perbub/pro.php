...
if (isset($_POST['simpan'])) {
    ...
    if ($tipe_file == "application/pdf") {
        ...
        $sql = "INSERT INTO tb_perbup  (nama, tahun, subag) VALUES ('$nama', '$tahun','$subag')";
        mysqli_query($koneksi, $sql); //simpan data judul dahulu untuk mendapatkan id

        //dapatkan id terkahir
        $query = mysqli_query($koneksi, "SELECT nama, tgl_upload FROM tb_perbup  ORDER BY id_perbup DESC LIMIT 1");
        ...
        mysqli_query($koneksi, "UPDATE tb_perbup  SET file='$nama_baru' WHERE nama='$data[nama]' ");
//ganti nama file
        $query = mysqli_query($koneksi, "SELECT nama, tgl_upload FROM tb_perbup  ORDER BY id_perbup DESC LIMIT 1");
        $data = mysqli_fetch_array($query);
